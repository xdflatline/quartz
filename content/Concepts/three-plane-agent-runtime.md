...
detail: "Split runtime responsibilities into control plane (shared metadata, auth, deployment registry), orchestration plane (per-run durable control flow), and execution plane (where code actually runs)."
